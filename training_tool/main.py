...
class DataAnnotationTab(QWidget):
    ...
    def init_ui(self):
        ...
        # 图片显示区域
        self.image_label = QLabel("请点击左侧加载图片")
        ...
        # 操作提示
        self.info_label = QLabel("操作说明: 加载图片后，在图片上拖拽绘制矩形框进行标注")
    ...
    def load_images(self):
        ...
        if folder:
            ...
            for img_file in image_files:
                self.image_list.addItem(os.path.join(folder, img_file))
...
class ModelTrainingTab(QWidget):
    ...
    def browse_yaml(self):
        ...
        file_path, _ = QFileDialog.getOpenFileName(
            self, "选择数据配置文件", "", "YAML Files (*.yaml *.yml)")
        if file_path:
    ...
    def start_training(self):
        ...
        self.training_thread = TrainingThread(
            data_path, model_type, epochs, batch_size, img_size
        )
...
class ModelExportTab(QWidget):
    ...
    def save_config(self):
        ...
        try:
            ...
            save_path, _ = QFileDialog.getSaveFileName(
                self, "保存配置文件", "objects.json", "JSON Files (*.json)")
            if save_path:
                with open(save_path, 'w', encoding='utf-8') as f:
