...
本应用支持**无需重新构建APK**就能更新识别模型和物体配置，通过以下方式同步：

1. **网络同步**：从服务器下载模型和配置文件
2. **本地同步**：从本地文件选择模型和配置文件

### 使用步骤

1. **在主界面点击"同步模型"按钮**
2. **选择同步方式**：
   - **服务器同步**：输入服务器地址（如 `http://192.168.1.100:8080/models`）
   - **本地同步**：选择 `.tflite` 模型文件和 `objects.json` 配置文件
3. **等待同步完成**
4. **应用自动更新模型**并开始使用新模型进行识别
